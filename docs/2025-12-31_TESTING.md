...
**Parser Smoke Tests** (`CerbLean/Test/Parser.lean`)
- Tests that malformed JSON is rejected
- Tests that incomplete JSON is rejected
- Tests that minimal valid JSON is accepted
- Run with: `make test-unit` or `.lake/build/bin/cerblean_test`

**Memory Model Tests** (`CerbLean/Test/Memory.lean`)
...
- Run with: `make test-memory` or `.lake/build/bin/cerblean_memtest`
...
├── Test.lean            # Unified test runner (cerblean_test)
├── TestBatch.lean       # Batch parser CLI (cerblean_testbatch)
├── TestPrettyPrint.lean # PP comparison CLI (cerblean_pp)
└── TestMemory.lean      # Memory test CLI (cerblean_memtest)

scripts/
├── test_parser.sh       # Parser test script
├── test_pp.sh           # Pretty-printer test script
├── test_pp_category.sh  # Category-specific PP tests
└── find_pp_mismatches.sh # Categorize PP mismatches
```
...
|------------|---------|-------------------|
| `cerblean_test` | Run all unit tests | No |
| `cerblean_memtest` | Run memory tests only | No |
| `cerblean_testbatch` | Batch parse JSON files | No (Lean-side) |
| `cerblean_pp` | Pretty-print/compare JSON | No (Lean-side) |
...
2. Run Lean parser on the JSON: `.lake/build/bin/cerblean_testbatch out.json`
...
2. Use the comparison tool for details:
   ```bash
   .lake/build/bin/cerblean_pp /path/to/file.json --compare /path/to/file.cerberus.core
   ```
...
Add test functions to `CerbLean/Test/Memory.lean`:
...
For smoke tests, add to `CerbLean/Test/Parser.lean`.
