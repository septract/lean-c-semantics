...
/-! ## Load Bridge Lemma
    ...
    If a MemValue's HeapValue has type τ, then the loaded Value
    (from `valueFromMemValue`) also has type τ. This bridges the
    HeapValue→Value direction needed for `action_load` soundness.
    The reverse direction (`valueHasType_implies_heapValueHasType`)
    is in HasType.lean and bridges Value→HeapValue for stores. -/

open CerbLean.Core (MemValue)
open CerbLean.Semantics (valueFromMemValue)

/-- HeapValue type implies loaded Value type (for load soundness).
    If `heapValueOfMemValue mv` has type τ, then `valueFromMemValue mv` has type τ.
    Proof: case analysis on MemValue constructors, checking that each
    HeapValue→Value conversion preserves the type relationship. -/
theorem heapValueHasType_implies_loadedValueHasType
    {mv : MemValue} {τ : CNBaseType}
    (hhv : heapValueHasType (CerbLean.CN.Semantics.heapValueOfMemValue mv) τ) :
    valueHasType (valueFromMemValue mv) τ := by
  sorry

/-! ## PexprMatchesTerm Eval Compatibility (strengthened)

    Replaces the old `pexprMatchesTerm_eval_compat` from HasType.lean.
    Uses `evalPexpr` directly instead of the limited `pexprEnvLookup`,
    covering non-symbol value expressions (intVal, bitsVal, op, etc.)
    needed for store soundness with computed value expressions. -/

/-- PexprMatchesTerm + EnvCompat ⟹ evalPexpr result is compatible with evalIndexTerm.
    If a Core Pexpr matches an IndexTerm (syntactic), the interpreter's scoped
    environment is compatible with the typing context (EnvCompat), and evalPexpr
    succeeds, then evalIndexTerm agrees on the result type.

    This is the key bridge between the syntactic `PexprMatchesTerm` and
    actual semantic evaluation. Needed for load, store, kill, proc rules. -/
theorem pexprMatchesTerm_eval_compat'
    {pe : CerbLean.Core.Pexpr} {it : CerbLean.CN.Types.IndexTerm}
    {ρ : CerbLean.CN.Semantics.Valuation}
    {env : List (Std.HashMap Sym Value)} {vars : List (Sym × CerbLean.CN.Types.BaseType)}
    {fuel : Nat}
    (_hmatch : PexprMatchesTerm pe it)
    (_hcompat : EnvCompat env vars ρ) :
    ∀ v (r : CerbLean.Semantics.InterpEnv) (s s' : CerbLean.Semantics.InterpState),
      ((evalPexpr fuel env ⟨[], none, pe⟩).run r).run s = .ok (v, s') →
      ∀ τ, valueHasType v τ →
        ∃ hv, CerbLean.ProofSystem.evalIndexTerm ρ it = some hv ∧
          heapValueHasType hv τ := by
  sorry

/-! ## condTermOfPexpr Correctness

    Bridges the interpreter's boolean evaluation of a condition Pexpr
    with the logical constraint generated by `condTermOfPexpr`.
    Needed for `if_` rule soundness (path condition correctness). -/

open CerbLean.ProofSystem (condTermOfPexpr evalConstraint)

/-- If `condTermOfPexpr pe = some condTerm` and `evalPexpr` evaluates `pe` to `.true_`,
    then `evalConstraint ρ (.t condTerm)` holds under a compatible valuation.
    Proof: case analysis on `condTermOfPexpr`'s pattern matching (sym, not_, op). -/
theorem condTermOfPexpr_correct
    {pe : CerbLean.Core.Pexpr} {condTerm : CerbLean.CN.Types.IndexTerm}
    (hcond : condTermOfPexpr pe = some condTerm)
    {env : List (Std.HashMap Sym Value)} {vars : List (Sym × CerbLean.CN.Types.BaseType)}
    {ρ : CerbLean.CN.Semantics.Valuation}
    {fuel : Nat}
    (_hcompat : EnvCompat env vars ρ)
    {r : CerbLean.Semantics.InterpEnv} {s s' : CerbLean.Semantics.InterpState}
    (_heval : ((evalPexpr fuel env ⟨[], none, pe⟩).run r).run s = .ok (Value.true_, s')) :
    evalConstraint ρ (.t condTerm) := by
  sorry
