...
partial def termToSmtTerm : Types.Term → TranslateResult
  ...
  | .wrapI _intType val =>
    ...
    -- For SMT, this is essentially a modular reduction (bvmod for bitvecs)
    match annotTermToSmtTerm val with
    | .unsupported r => .unsupported r
    | .ok valTm =>
      -- wrapI is a modular wrap - for bitvec types the SMT sort enforces this
      -- For unbounded integers, we need explicit modular arithmetic
      -- wrapI's intType is a Core.IntegerType, but at the SMT level the value
      -- is already declared with the right BitVec sort, so this is identity
      .ok valTm
  | .cast targetType val =>
    ...
    | .ok valTm =>
      ...
      | .loc, .bits _ tw =>
        ...
        .ok (Term.appT (Term.symbolT (s!"(_ int2bv {tw})")) valTm)
      | _, _ =>
        -- For same-sort casts, identity (e.g., integer→integer, bits→bits)
        .ok valTm
  ...
  | .sizeOf ct =>
    ...
    | .basic (.integer ity) =>
      let size := match ity with
        | .bool => 1
        | .char => 1
        | .signed .ichar | .unsigned .ichar => 1
        | .signed .short | .unsigned .short => 2
        | .signed .int_ | .unsigned .int_ => 4
        | .signed .long | .unsigned .long => 8
        | .signed .longLong | .unsigned .longLong => 8
        | .signed .intptr | .unsigned .intptr => 8
        | .size_t | .ptrdiff_t | .ptraddr_t => 8
        | .wchar_t | .wint_t => 4
        | .enum _ => 4
        | _ => 0
      if size > 0 then .ok (Term.literalT (toString size))
      else .unsupported s!"sizeOf ({repr ct.ty})"
  ...
  | .match_ scrutinee cases =>
    ...
    | (pat, body) :: rest =>
      ...
      let bindings := extractPatternBindings pat scrutinee
        ...
        if bindings.isEmpty then
          ...
          if rest.isEmpty then .ok bodySmtTm
            ...
            | [(_, body2)] =>
              ...
              | .unsupported r, _ => .unsupported r
              | _, .unsupported r => .unsupported r
            | _ => .ok bodySmtTm  -- >2 cases: use first
          else .ok bodySmtTm  -- Non-boolean multi-case: use first
        else
...
/-- Extract pattern bindings from a match: pairs of (smt_name, scrutinee_component_term).
    ...
    Returns empty list if no bindings can be extracted. -/
partial def extractPatternBindings (pat : Types.Pattern) (scrutinee : AnnotTerm) : List (String × Smt.Term) :=
  match pat.pat with
  | .sym s =>
    -- Single variable pattern: bind to whole scrutinee
    match annotTermToSmtTerm scrutinee with
    | .ok tm => [(symToSmtName s, tm)]
    | .unsupported _ => []
  | .constructor _ctor args =>
    ...
    | .tuple components =>
      ...
      pairs.flatMap fun ((_, subPat), component) =>
