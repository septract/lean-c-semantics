...
partial def bindPattern (pat : APattern) (value : IndexTerm) : TypingM PatternBindings := do
  ...
  | .base (some sym) bt =>
    ...
    let cnBt ← match coreBaseTypeToCN bt with
      | some t => pure t
      | none =>
        TypingM.fail (.other s!"Unsupported Core base type in pattern binding: {repr bt}")
...
partial def checkPexpr (pe : APexpr) (expectedBt : Option BaseType := none) : TypingM IndexTerm := do
  ...
  | .areCompatible e1 e2 =>
    ...
    | .const (.ctypeConst ct1), .const (.ctypeConst ct2) => return boolLit (ct1 == ct2) loc
    | _, _ => TypingM.fail (.other "are_compatible: arguments are not concrete ctypes")
  ...
  | .cfunction e =>
    ...
    match ← TypingM.lookupFunInfo funSym.id with
    | some fi =>
