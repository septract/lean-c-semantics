...
/-- Run a computation catching errors. On success, preserve obligations and
    conditional failures. On failure, return the error as a value without
    propagating it. State is unchanged on failure (inner CFs lost, but subsumed
    by the outer CF the caller creates).

    This is the error-catching variant of pure_.
...
def tryPure (m : TypingM α) : TypingM (Except TypeError α) := fun s =>
  match m s with
  | .ok (val, newState) =>
    .ok (.ok val, { s with
      obligations := newState.obligations
...
/-- Look up function info by symbol ID.
    Used by cfunction(), params_length, etc. for concrete evaluation.
    Corresponds to: looking up function info in Core file -/
def lookupFunInfo (symId : Nat) : TypingM (Option Core.FunInfo) := do
  let s ← getState
  -- FunInfoMap is keyed by Sym, so we search by ID
  let result := s.funInfoMap.toList.findSome? fun (sym, fi) =>
    if sym.id == symId then some fi else none
  return result
