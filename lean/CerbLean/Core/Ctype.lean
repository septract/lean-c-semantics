...
-- IntBaseKind and IntegerType are now defined in IntegerType.lean
-- (imported transitively via Annot.lean)
-- enum tag uses Nat (symbol ID) instead of Sym to break the import cycle
