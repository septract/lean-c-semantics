// Adapted from cerberus/tests/csmith/small_int_arith/csmith_194.c
// Uses csmith_cerberus.h which exits with checksum as return code
//
// Original csmith options:
//   --no-argc --no-arrays --no-bitfields --checksum --concise --no-consts
//   --no-float --no-inline-function --max-array-dim 3 --max-array-len-per-dim 10
//   --max-block-depth 5 --max-block-size 4 --max-expr-complexity 2 --max-funcs 1
//   --max-pointer-depth 2 --no-pointers --no-structs --no-unions --no-volatiles
//   --no-builtins
...
int main(void)
{
    ...
    return 0;  // Never reached - platform_main_end calls exit()
}
